...
- 코드 변경 전에 `git fetch origin`을 실행하고 `origin/main`을 merge해 동기화하라. 이슈 브랜치가 로컬이나 `origin`에 있으면 그 브랜치를 체크아웃한 뒤 동기화하라. 강제 푸시가 승인되지 않으므로 rebase하지 마라.
...
- `$git-branch`로 이슈 `{{ issue.identifier }}`의 브랜치를 만들고 변경사항을 커밋하라. 이슈 브랜치가 이미 있으면 새로 만들지 말고 `$git-commit`으로 커밋하라. 게시 전에 `origin/main`을 다시 merge하고, 동기화로 브랜치가 바뀌면 검증과 코드 리뷰를 반복하라.
