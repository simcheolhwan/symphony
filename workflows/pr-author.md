...
- 스킬은 적용 대상을 스스로 정하지 않는다. 호출할 때 대상 PR과 head를 명시하라. 답글과 PR 코멘트는 초안 승인 없이 바로 게시하고, 필수 정보나 권한이 없으면 질문하지 말고 차단 요인으로 보고하라.

## 절차

1. `git fetch origin` 후 `{{ issue.branch_name }}`을 `origin`의 head로 체크아웃하라.
2. `$github-resolve-feedback`로 PR `{{ issue.id }}`의 미해결 피드백과 완료된 CI 실패를 처리하라. 기준은 `origin`의 현재 head SHA다. 무인 실행이므로 다음 결정을 그대로 적용하라.
   - `유효` 판정 전부를 수정 대상으로 확정하라. `모호함`은 안전한 최소 해석이 가능하면 해석을 답글에 남기고 수정하고, 요구사항을 추측해야 하면 차단 요인으로 보고하라.
   - 자동 검증이 통과하면 사용자 확인 없이 즉시 커밋하고 head 브랜치를 `origin`에 푸시하라.
   - 진행 중인 check는 기다리지 마라. 리뷰봇이 자기 지적의 수정 커밋을 자동 생성하는 check는 결과를 무시하고 그 피드백을 직접 처리하라.
   - 무효이거나 이미 수정된 지적 사항에는 근거를 답변하라.
