...
// 발신(Erlang)과 수신(Node)의 IPv4/IPv6 주소 해석 결과가 다를 수 있어 배제한다.
...
const envSchema = z.object({
  ...
  SYMPHONY_SLACK_CHANNEL: z.string().regex(/^[CDG][A-Z0-9]+$/, "Slack 대화 ID 형식이어야 합니다."),
  // 잘못된 ID는 Slack이 오류 없이 평문으로 출력해 조용히 실패하므로 형식을 여기서 검증한다.
