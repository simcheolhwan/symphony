...
| `SYMPHONY_SLACK_BOT_TOKEN` | Slack Bot 토큰 (`chat:write` scope)                                                                                                                                      |
| `SYMPHONY_SLACK_CHANNEL`   | 알림을 게시할 대화 ID. `C`(공개 또는 비공개 채널), `G`(그룹 DM), `D`(DM)로 시작하는 대문자와 숫자 형식만 허용한다. 이름은 허용하지 않는다                                |
| `SYMPHONY_SLACK_USER_ID`   | 스레드 답글에서 멘션할 사용자 ID. `U` 또는 `W`(Enterprise Grid)로 시작하는 대문자와 숫자 형식만 허용한다                                                                 |

수신 포트를 별도 환경변수로 두면 발신 URL의 포트와 어긋나도 발신 측의 연결 실패 로그로만 드러난다. URL 하나에서 도출해 그 가능성을 없앤다. `http` 스킴, 호스트 `127.0.0.1`, 0보다 큰 명시적 포트, 사용자 정보가 없는 URL만 허용한다. 명시한 기본 포트 `:80`도 허용한다. 서버가 `127.0.0.1`에 바인딩하므로 호스트도 그 표기 하나만 허용하며, `localhost`는 발신과 수신의 IPv4/IPv6 주소 해석 결과가 다를 수 있어 배제한다. 양끝 공백을 제거한 뒤 발신 측(오케스트레이터)과 같은 규칙으로 검증해, 어긋난 값이면 수신 측은 부팅을 거부하고 발신 측은 이벤트마다 경고를 남긴 뒤 발신을 건너뛴다.
...
| `issue.url`        | string            | 트래커 이슈의 HTTP(S) URL                                                                                                                                                                                        |
...
- `finished`: 워크플로의 순조로운 종결. 비활성 상태나 종료 상태로의 정상 전환과 PR 머지다.
...
서사(수행 내용 요약)는 사실이 아니라 에이전트의 자기 보고이고, `agent_message` 필드로 전문이 그대로 전달된다. 이 서버는 내용을 해석하거나 판정하지 않고 첫 줄만 `에이전트 요약`으로 표기해 답글에 덧붙인다. 사실과 자기 보고를 표기로 구분해 두어야 잘못된 자기 보고가 관측 결과로 읽히지 않는다.

모든 문자열 필드와 문자열 배열 항목은 제공됐을 때 공백이 아닌 문자를 하나 이상 포함해야 한다. 스키마는 값을 정규화하지 않으므로 검증을 통과한 문자열은 원문 그대로 템플릿에 전달된다.

`event`는 열린 집합이다. Zod 스키마는 위 빈 값 규칙만 적용하고, 정의된 종류가 아니면 기본 템플릿으로 게시한다. 발신 측이 종류를 추가해도 수신 측 수정이 필요 없다.
...
- 매핑 키는 `${instance}:${issue.id}`이고, 값은 본문 메시지의 `ts`다. `ts`는 `1700000000.000100`처럼 초 단위 정수부와 6자리 소수부 형식만 허용한다. 로컬 JSON 파일 `~/.config/symphony/notifier-threads.json`으로 영속화해 알림 서버가 재시작해도 같은 작업이 같은 스레드를 쓴다.
...
- 어떤 이벤트도 매핑을 제거하지 않는다. 실행 완료 후 재디스패치가 흔한 PR 워크플로에서 완료 시 제거는 다음 실행을 새 본문으로 보내 스레드를 쪼갠다. 파일 크기는 항목 수 상한 500으로 정리하고, 삽입 순서가 오래된 항목부터 축출한다. 상한은 새 본문을 게시하는 시점에만 적용되고 로드 시점에는 정리하지 않는다. 상한 초과로 축출된 작업의 이벤트는 새 본문으로 시작한다.
- 매핑 파일이 없거나 JSON 또는 최상위 형식이 손상됐으면 빈 상태로 시작한다. 개별 항목의 `ts`만 손상됐으면 그 항목을 버리고 나머지는 복원한다. 알림은 유실을 허용하는 보조 채널이라 매핑을 잃은 작업은 새 본문으로 다시 시작하면 된다.
...
- 답글의 `markdown` 블록: `agent_message`가 있으면 `section` 블록 뒤에 `↳ **(에이전트 요약)** {첫 줄}`을 담은 블록을 잇는다. 첫 번째 비어 있지 않은 줄만 쓰고 500자를 넘으면 `…`로 절단한다. 첫 줄을 요약으로 쓰라는 요구는 워크플로 프롬프트가 명시하지만 에이전트가 어길 수 있으므로 표현 계층에서 한 번 더 자른다. 절단 상한이 `markdown` 블록의 12,000자 한도보다 훨씬 낮아 한도는 고려하지 않는다.
...
답글에만 멘션을 붙인다. 스레드를 팔로우하지 않으면 답글 알림이 오지 않아 차단, 중단이 묻히기 때문이다. 본문은 대상 대화의 새 메시지라 이미 알림 대상이다.
...
▶️ *Linear (acme/web)* <https://linear.app/acme/issue/SYM-57|SYM-57> 결제 웹훅 재시도 정책 적용
  ...
  └ ⛔ 차단 (In Progress) – 검증 불가: 스테이징 웹훅 시크릿 미제공
      ↳ **(에이전트 요약)** 재시도 정책 구현은 마쳤지만 통합 테스트가 스테이징 시크릿을 요구해 검증을 끝내지 못했다. 이슈 코멘트에 차단 사유를 기록했다.
...
src/
  ...
  config.ts      # 환경변수 파싱과 검증, 상태 확인 인터페이스
  slack.ts       # WebClient 래퍼, 키별 직렬 게시, 스레드 매핑과 영속화
  events.ts      # 라이프사이클 이벤트 라우트: 페이로드 스키마, 게시 흐름
  templates.ts   # 본문 텍스트와 답글 블록 템플릿 (순수 함수)
```

순수 함수인 설정 파싱, 페이로드 스키마, 템플릿, 매핑 직렬화와 축출, Slack 응답 스키마는 Vite+로 같은 디렉터리에서 테스트한다. HTTP 서버, 실제 Slack 호출, 파일 입출력은 통합 지점이 얇아 테스트 대상에서 제외한다.
...
- 요청 인증: 루프백 바인딩이 보안 경계다. 수신을 루프백 밖으로 넓히면 그때 도입한다.
