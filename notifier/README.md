...
- `finished`: 워크플로의 순조로운 종결. 비활성 상태나 terminal 상태로의 정상 전환과 PR 머지다.
...
- 매핑 키는 `${instance}:${issue.id}`이고, 값은 본문 메시지의 `ts`다. `ts`는 `1700000000.000100`처럼 초 단위 정수부와 6자리 소수부 형식만 허용한다. 로컬 JSON 파일 `~/.config/symphony/notifier-threads.json`로 영속화해 알림 서버가 재시작해도 같은 작업이 같은 스레드를 쓴다.
...
순수 함수인 설정 파싱, 페이로드 스키마, 템플릿, 매핑 직렬화와 축출, Slack 응답 스키마는 Vite+로 같은 디렉터리에서 테스트한다 (저장소 루트에서 `pnpm test`). HTTP 서버, 실제 Slack 호출, 파일 입출력은 통합 지점이 얇아 테스트 대상에서 제외한다.
